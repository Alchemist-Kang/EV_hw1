...
from Individual2 import Individual
...
#  2-D Rastrigin function       multi-variate real-number upgrade
def fitnessFunc(x):
    A=10
    n=2
    loss_value=0.0
    loss_value=loss_value + A*n
    for item in x:
        loss_value = loss_value + (item)**2 -A*math.cos(2*math.pi*(item))
