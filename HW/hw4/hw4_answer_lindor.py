...
#          - allow more children per generation (we move towards a so-called generational model)
...
class EV2_Config:
    ...
    #number of children per generation (should probably be an input cfg param, but...)
    numChildren=5
...
#Simple 1-D fitness function example: 1-D Rastrigin function
#        
def fitnessFunc(x):
...
#EV2: EV1 with self-adaptive mutation & stochastic crossover, multiple children per gen
#            
def ev2(cfg):
    ...
    for i in range(cfg.generationCount):
        children=[]
        for j in range(cfg.numChildren):
